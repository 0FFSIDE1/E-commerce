...
class VendorReview(models.Model):
    customer = models.ForeignKey(Customer, default=None, blank=False, null=False, on_delete=models.CASCADE)
    vendor = models.ForeignKey(Vendor, default=None, blank=False, null=False, on_delete=models.CASCADE)
    message = models.TextField(max_length=200, default=None, blank=False, null=False)
    rating = models.DecimalField(max_digits=3, decimal_places=1)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"Vendor rating: {self.rating} | {self.order.order_id} | {self.customer.first_name}"
