...
class SubscriptionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Subscription
        fields = ['id', 'user', 'plan', 'user_name', 'plan_name', 'plan_details', 'start_date', 'expire_date', 'is_active', 'remaining_days']
        read_only_fields = ['start_date', 'expire_date', 'is_active', 'remaining_days', 'user',]

    def validate(self, data):
        # Ensure only one active subscription per user
        if Subscription.objects.filter(user=self.context['request'].user.vendor, is_active=True).exists():
            raise serializers.ValidationError("User already has an active subscription.")
        return data
    

class RenewSubscriptionSerializer(serializers.ModelSerializer):
    plan_details = SubscriptionPlanSerializer(source='plan', read_only=True)
    user_name = serializers.CharField(source='user.username', read_only=True)  # Fetch the full name of the user
    plan_name = serializers.CharField(source='plan.name', read_only=True)          # Fetch the name of the plan
    start_date = serializers.DateTimeField(format='%Y-%m-%d', read_only=True)
    expire_date = serializers.DateTimeField(format='%Y-%m-%d', read_only=True)
