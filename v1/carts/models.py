...
class Cart(models.Model):
    ...
    session_id = models.OneToOneField('sessions.Session', max_length=255, null=True, blank=True, on_delete=models.CASCADE, help_text="Session ID for anonymous users")  # This is the logic for anonymous users
    customer = models.OneToOneField(Customer, on_delete=models.CASCADE, null=True, blank=True, help_text="Customer who owns this cart")
    created_at = models.DateTimeField(auto_now_add=True)
    modified_at = models.DateTimeField(auto_now=True, blank=True, null=True)

    def __str__(self):
        return self.customer.email if self.customer else f"session id: {self.session_id}"
